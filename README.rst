...
``BLUEFOG_WITH_NCCL=1 pip install bluefog`` if NCCL is supported (``NCCL>=2.7``).It should be noted that after installation, please check if the header file can be seen by the compiler.Use ``ls -l /usr/lib/libnccl* `` or ``ls -l /usr/local/nccl-<version>/lib/libnccl* `` to confirm . Check
