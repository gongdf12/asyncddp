...
void MPIContextManager::EnvInitialize(int mpi_threads_required) {
  ...
  BFLOG(INFO) << "MPI 环境初始化: 请求线程级别=" << mpi_threads_required
              << ", 提供线程级别=" << mpi_threads_provided;
}
...
void MPIContext::Initialize(const std::vector<int>& ranks,
  ...
  if (is_mpi_initialized) {
    ...
    BFLOG(INFO) << "MPI 已初始化, 线程支持级别=" << provided;
  } else {
  ...
  int local_rank_dbg = -1, local_size_dbg = -1, cross_rank_dbg = -1, cross_size_dbg = -1;
  // local_comm and cross_comm will be created below; sizes are logged once ready.
  ...
  MPI_Comm_size(local_comm, &local_size_dbg);

  // Create cross node communicator.
  MPI_Comm_split(mpi_comm, local_rank, world_rank, &cross_comm);
  MPI_Comm_rank(cross_comm, &cross_rank_dbg);
  MPI_Comm_size(cross_comm, &cross_size_dbg);
  ...
  BFLOG(INFO) << "MPIContext 初始化完成"
              << ", world_rank/size=" << world_rank << "/" << world_size
              << ", local_rank/size=" << local_rank << "/" << local_size_dbg
              << ", cross_rank/size=" << cross_rank_dbg << "/" << cross_size_dbg;
}
