...
void MPIController::Initialize() {
  ...
  BFLOG(INFO, mpi_ctx_.rank_) << "MPI 控制器初始化完成"
                              << ", 线程支持级别=" << provided
                              << ", 世界 rank/size=" << mpi_ctx_.rank_ << "/" << mpi_ctx_.size_
                              << ", 本地 rank/size=" << mpi_ctx_.local_rank_ << "/" << mpi_ctx_.local_size_
                              << ", 跨节点 rank/size=" << mpi_ctx_.cross_rank_ << "/" << mpi_ctx_.cross_size_;
}
...
void MPIController::Allreduce(TensorTableEntry& entry) {
  ...
  BFLOG(INFO, mpi_ctx_.rank_)
      << "Allreduce(MPI) begin name=" << entry.tensor_name
      << " elems=" << num_elements
  ...
  BFLOG(INFO, mpi_ctx_.rank_)
      << "Allreduce(MPI) done name=" << entry.tensor_name;
  entry.callback(Status::OK());
}
...
void MPIController::Allreduce(std::vector<TensorTableEntry>& entries) {
  ...
  BFLOG(INFO, mpi_ctx_.rank_)
      << "Allreduce(MPI,fused) begin count=" << entries.size()
  ...
  BFLOG(INFO, mpi_ctx_.rank_)
      << "Allreduce(MPI,fused) done count=" << entries.size()
      << " first_name=" << first_entry.tensor_name;
}
...
//检查发送的是否符合内存大小和类型的内容，否则会出现内存溢出和报错
 {
      auto target_tensor = win_mananger->GetAssociateTensorByRank(target_rank);
      int target_elems = target_tensor->shape().num_elements();
      if (target_elems != num_elements ||
          target_tensor->dtype() != entry.tensor->dtype()) {
        throw std::runtime_error(
            std::string("WinPut size/dtype mismatch for ") + entry.tensor_name +
  ...
  for (auto kv : sorted_dst_weights) {
...

    {
      ...
      if (target_elems != num_elements ||
          target_tensor->dtype() != entry.tensor->dtype()) {
        throw std::runtime_error(
            std::string("WinAccumulate size/dtype mismatch for ") + entry.tensor_name +
            ": send numel=" + std::to_string(num_elements) +
            ", target numel=" + std::to_string(target_elems));
      }
    }
...
void MPIController::WinReadNeighbor(TensorTableEntry& entry) {
  // 保证在正确 device 上做 memcpy（包括 GPU）
  with_device device_guard(entry.device);

  auto it = mpi_ctx_.named_win_map.find(entry.tensor_name);
  if (it == mpi_ctx_.named_win_map.end()) {
    throw std::runtime_error(
        std::string("Cannot find ") + entry.tensor_name +
        std::string(" in (MPI) registered win object name."));
  }
  std::shared_ptr<WindowManager> win_manager = it->second;

  if (!entry.output) {
    entry.callback(Status::InvalidArgument(
        "WIN_READ_NEIGHBOR requires non-null output tensor"));
    return;
  }

  if (entry.src_weights.size() != 1) {
    entry.callback(Status::InvalidArgument(
        "WIN_READ_NEIGHBOR expects exactly one source rank"));
    return;
  }
  int neighbor_rank = entry.src_weights.begin()->first;

  // 根据 require_mutex 决定是否在通信线程里加 MPI 互斥
  if (entry.require_mutex) {
    Status st =
        WinMutexAcquire(entry.tensor_name, {neighbor_rank}, /*is_sync=*/true);
    if (!st.ok()) {
      entry.callback(st);
      return;
    }
  ...
  std::shared_ptr<Tensor> src_tensor =
      win_manager->GetAssociateTensorByRank(neighbor_rank);

  if (!src_tensor) {
    if (entry.require_mutex) {
      (void)WinMutexRelease(entry.tensor_name, {neighbor_rank},
                            /*is_sync=*/true);
    }
    entry.callback(Status::InvalidArgument(
        "WIN_READ_NEIGHBOR cannot get window tensor for rank " +
        std::to_string(neighbor_rank)));
    return;
  }

  // 基本尺寸 / dtype 校验，防止内存越界
  if (src_tensor->dtype() != entry.output->dtype() ||
      src_tensor->size() != entry.output->size()) {
    if (entry.require_mutex) {
      (void)WinMutexRelease(entry.tensor_name, {neighbor_rank},
                            /*is_sync=*/true);
    }
    entry.callback(Status::InvalidArgument(
        "WIN_READ_NEIGHBOR size/dtype mismatch for " + entry.tensor_name));
    return;
  }

  // 这里直接复用已有的 CPU/GPU memcpy 封装
  MemcpyEntryOutFusionBuffer(src_tensor->data(), entry);

  if (entry.require_mutex) {
    Status st =
        WinMutexRelease(entry.tensor_name, {neighbor_rank}, /*is_sync=*/true);
    if (!st.ok()) {
      entry.callback(st);
      return;
    }
