...
void NCCLContext::Initialize(const int rank, const int size,
  ...
  BFLOG(INFO) << "NCCL 上下文初始化开始"
              << ", rank=" << rank << "/" << size
              << ", local_rank=" << local_rank << "/" << local_size;
  self_rank = rank;
...
void NCCLController::Initialize() {
...
#endif
  BFLOG(INFO, mpi_ctx_.rank_) << "NCCL 控制器初始化: 创建最终回调线程池";
  ...
  BFLOG(INFO, mpi_ctx_.rank_) << "NCCL 控制器初始化完成, finalizer_threads="
                              << num_finalizer_threads;
}
...
void NCCLController::Allreduce(TensorTableEntry& entry) {
  ...
  BFLOG(INFO, mpi_ctx_.rank_)
      << "Allreduce(NCCL) launch name=" << entry.tensor_name
      << " elems=" << num_elements
      << " dtype=" << DataType_Name(entry.tensor->dtype())
      << " scope=" << (entry.is_hierarchical ? "LOCAL" : "GLOBAL");
  ...
  nccl_ctx_.finalizer_thread_pool.execute(
      [this, entry, event_queue, tid]() mutable {
        ...
        this->timeline_ptr_->ActivityEnd(entry.tensor_name, &tid);
        BFLOG(INFO, mpi_ctx_.rank_)
            << "Allreduce(NCCL) done name=" << entry.tensor_name;
      });
}
...
void NCCLController::Allreduce(std::vector<TensorTableEntry>& entries) {
  ...
  BFLOG(INFO, mpi_ctx_.rank_)
      << "Allreduce(NCCL,fused) launch count=" << entries.size()
      << " total_elems=" << num_elements
      << " first_name=" << first_entry.tensor_name
      << " dtype=" << DataType_Name(first_entry.tensor->dtype())
      << " scope=" << (first_entry.is_hierarchical ? "LOCAL" : "GLOBAL");
  ...
  nccl_ctx_.finalizer_thread_pool.execute(
      [this, entries, event_queue, tid]() mutable {
        ...
        BFLOG(INFO, mpi_ctx_.rank_)
            << "Allreduce(NCCL,fused) done count=" << entries.size()
            << " first_name=" << first_entry.tensor_name;
      });
}
...
// void WinPassiveRecvRequest1(int self_rank, NCCLContext& nccl_ctx ) {
//   BFLOG(DEBUG, self_rank) << "WinPassiveRecvRequest thread initialized";
...
//     int skipped_rounds;
//   };
//   std::deque<PendingReq> pending; // 定义一个消息结构体用于有序处理到队列的消息，防止死锁
//   // int in_world_size = (int)mpi_ctx.neighbor_in_ranks_.size() ;
...
//         bluefog_load_topology(&in_degree, sources, &out_degree, destinations);
//         for (int i = 0; i < in_degree; i++) { //收集数据每个in节点都收一次
...
//               int rc= MPI_Irecv(req_buf.data(), 4, MPI_INT, MPI_ANY_SOURCE,
//                         kWinPassiveRecvRequestTag, MPI_COMM_WORLD, &mpi_request);
//               if (rc != MPI_SUCCESS) {
//                   char errstr[MPI_MAX_ERROR_STRING];
//                   int len = 0;
//                   MPI_Error_string(rc, errstr, &len);
//                   BFLOG(ERROR, self_rank) << "MPI_Irecv posting failed: " << errstr;
//                   // 根据需要 return / continue
//                   continue;
//                 } //这个是在检查irecv本身是否调用成功
...
//                 if (!mpi_flag) { //等待，超过10微秒不等；
//                     if (wait_time > 10) {
//                       BFLOG(DEBUG, self_rank) << "没有接收到消息,跳出循环";
//                       // 取消接收请求
...
//                 PendingReq pending_req={source, req, skipped_rounds};
//                 pending.push_back(pending_req);
//                 // 发送已经接收消息
//                 int ready = BFWinPassiveSuccess;
//                 MPI_Send(&ready, 1, MPI_INT, source,
//                               kWinPassiveRecvAckTag, MPI_COMM_WORLD);

//                 // 将接收的请求加入队列
//                   BFLOG(DEBUG,self_rank)<<"passive rank"<<self_rank
//                   <<"have rev mpi req,the source is"<<mpi_status.MPI_SOURCE
//                   <<"the deque len is"<<pending.size();
...
//                   MPI_Cancel(&mpi_request);
//                   BFLOG(DEBUG, self_rank) << "WinPassiveRecvRequest is shutting down.";
//                   break;
//                 }
            
//         }
//     }  //接收消息完毕，开始处理队列中的消息。
//     BFLOG(DEBUG, self_rank) << "开始处理这一轮消息，消息接收完毕 ";
                            
    



//     // 2) 如果 pending 为空，就继续下一轮循环（这里只做收）
//     if (pending.empty()) {
//        std::this_thread::sleep_for(std::chrono::microseconds(1));
//       continue;
//     }

//     // 3) 从 pending 中选一个要处理的：
//     //    例如“skipped_rounds 最大”的那个
//     auto best_it = pending.begin();
//     for (auto it = pending.begin(); it != pending.end(); ++it) {
//       if ((it->skipped_rounds+it->source) > (best_it->skipped_rounds+best_it->source)) {
//         best_it = it;
//       }
//     }
//     PendingReq current = *best_it;
//     // pending.erase(best_it);
//     for (auto& item : pending) {item.skipped_rounds++;}

//     BFLOG(DEBUG, self_rank) << "还剩下的消息条数 " << self_rank
//                             << ", pending size=" << pending.size();
//     int source = current.source;
//     NCCLWinRequest req = current.req;
   
//     // 上锁之前先进行避让，避让时间取决于 rank编号
//     unsigned int delay_us =  self_rank*1000;  // 例如 0~9 微秒
//     std::this_thread::sleep_for(std::chrono::microseconds(delay_us)); //在这个等待时间内win_put需要完成信息通信的所以工作并且开始上锁，否则会死锁
    
//     while(true){ //确保每一条消息都会有一次拥锁机会
//         if (nccl_ctx.nccl_win_mutex.try_lock()) {
//           res_buf[0] = BFWinPassiveSuccess;   // 给源rank发送拥锁消息
//           MPICHECK(MPI_Send(res_buf.data(), 1, MPI_INT, source,
//                             kWinPassiveRecvAckTag, MPI_COMM_WORLD));
//           comm_thread_status.ready_passive = 1;
//           BFLOG(DEBUG, self_rank) << "Successfully passive locked,the rank is"
//                                   << self_rank<<"the source is "<<source;
//           break;
...
//           // MPICHECK(MPI_Send(res_buf.data(), 1, MPI_INT, source,
//           //                   kWinPassiveRecvAckTag, MPI_COMM_WORLD));
//           BFLOG(DEBUG, self_rank) << "Due to lock, unable to process request from "
//                                   << source ;
//           unsigned int delay_us = 5 + self_rank*10;  // 例如 0~9 微秒
//           std::this_thread::sleep_for(std::chrono::microseconds(delay_us));
//           continue;
//         }
//     }
//     //已经有了win锁，还有读写互斥检查
//     while(win_mutex.global ==1){
//       //有全局禁止量
//       std::this_thread::sleep_for(std::chrono::microseconds(2)); 
//     }
//     win_mutex.use =1; //表明有写入操作
//     win_mutex.target_rank = source; //目标rank


//     std::string win_name = nccl_ctx.window_id_manager.GetNameById(req.name_id); 
//     auto it = nccl_ctx.named_win_map.find(win_name);
//     if (it == nccl_ctx.named_win_map.end()) {
//       BFLOG(ERROR, self_rank)
//           << "WinPassiveRecvRequest cannot find window '" << win_name
//           << "' in named_win_map";
//       nccl_ctx.nccl_win_mutex.unlock();
//       continue;
//     }
//     std::shared_ptr<NCCLWindowManager> nccl_win_manager = it->second;
//     // int win_device = nccl_win_manager->GetWinMemoryDevice();
...
//       nccl_ctx.nccl_win_mutex.unlock(); //获取关键数据后，就释放锁，减少锁的粒度
//       #if NCCL_MINOR > 6 //代表编译的版本号
...
//             BFLOG(DEBUG, self_rank) << "PASSIVE RECV NCCL ,rank"<<self_rank;
//       #else
...
//     }
//     // 全局变量修改
//     win_mutex.target_rank = -1; 
//     win_mutex.use =0;
//     // nccl_ctx.nccl_win_mutex.unlock();
//     pending.erase(best_it);
//     BFLOG(DEBUG, self_rank)<<"RELEASE LOCK,RANK"<<self_rank;
//     nccl_ctx.finalizer_thread_pool.execute([win_stream, req, source,self_rank]() mutable {
//       CUDACHECK(cudaStreamSynchronize(win_stream));//将流的完成单开一个线程处理

//       // NEW: 打印被动侧 NCCL 流完成
//       BFLOG(DEBUG, self_rank)
//           << "WinPassiveRecvRequest NCCL stream sync done for window_id="
//           << req.name_id
//           << " op_type=" << to_underlying(req.op_type)
//           << " source=" << source
//           << " (self_rank=" << self_rank << ")";
...
//           // NEW: 打印 done 已发送
//             BFLOG(DEBUG, self_rank)
//                 << "WinPassiveRecvRequest sent done to source=" << source
//                 << " for window_id=" << req.name_id
//                 << " (self_rank=" << self_rank << ")";
//           }
//     });
//   }
...
void WinPassiveRecvRequest(int self_rank, NCCLContext& nccl_ctx ) {
  ...
  std::vector<int> req_buf(4, -1);
  std::vector<int> res_buf(1, -1);
  cudaStream_t win_stream; //已经定义了win_stream
  while (!nccl_ctx.win_passive_recv_shutdown) {
        MPI_Status mpi_status;
        MPI_Request mpi_request;
        int mpi_flag;
        MPI_Irecv(req_buf.data(), 4, MPI_INT, MPI_ANY_SOURCE,
                        kWinPassiveRecvRequestTag, MPI_COMM_WORLD, &mpi_request);//开始接收消息
        while (!nccl_ctx.win_passive_recv_shutdown) {   
            MPI_Test(&mpi_request, &mpi_flag, &mpi_status);
            if (!mpi_flag) { //等待
                std::this_thread::sleep_for(std::chrono::microseconds(1));
        ...
        int source = mpi_status.MPI_SOURCE;
        NCCLWinRequest req = DeserializeNCCLWinRequest(req_buf);
        // int timewait = 10+self_rank * 20;  //退避时间，防止同时启动造成的死锁
        // std::this_thread::sleep_for(std::chrono::microseconds(timewait));
        //已经接收到消息，开始尝试上锁处理
        while (!nccl_ctx.win_passive_recv_shutdown) {
          // while(win_mutex.global ==1){
          //       //有全局禁止量
        ...
        // if(true){
          ...
          //   BFLOG(DEBUG, self_rank) << "Due to lock, unable to process request from "
          //                           << source ;
          //   unsigned int delay_us = 5 ;  // 例如 0~9 微秒
          //   std::this_thread::sleep_for(std::chrono::microseconds(delay_us));
          //   continue;
          // }
        }
      ...
      BFLOG(DEBUG, self_rank)<<"RELEASE LOCK,RANK"<<self_rank;
      nccl_ctx.finalizer_thread_pool.execute([win_stream, req, source,self_rank]() mutable {
            // std::lock_guard<std::mutex> guard(w_r_mutex);
            CUDACHECK(cudaStreamSynchronize(win_stream));//将流的完成单开一个线程处理
            if (req.op_type == MPIOpsType::WIN_ACCUMULATE ||
                req.op_type == MPIOpsType::WIN_PUT) {
                int done = 1;
...
void NCCLController::WinCreate(TensorTableEntry& entry) {
  ...
  // NEW: 打印 WinCreate 的基本信息
  BFLOG(DEBUG, mpi_ctx_.rank_)
      << "WinCreate (NCCL) begin: name=" << name
      << " device=" << entry.device
      << " tensor=" << (entry.tensor ? "non-null" : "null")
      << " neighbor_tensors.size=" << entry.neighbor_tensors.size()
      << " in_neighbor_ranks.size=" << mpi_ctx_.neighbor_in_ranks_.size();

  // 添加张量有效性检查
  if (!entry.tensor) {
    BFLOG(ERROR, mpi_ctx_.rank_)
        << "WinCreate (NCCL) tensor is null for " << name;
    entry.callback(Status::InvalidArgument("Win_create: tensor is null for " + name));
    return;
  }

  if (!entry.tensor->data()) {
    BFLOG(ERROR, mpi_ctx_.rank_)
        << "WinCreate (NCCL) tensor->data() is null for " << name;
    entry.callback(Status::InvalidArgument("Win_create: tensor data is null for " + name));
    return;
  }

  // 检查邻居张量
  if (entry.neighbor_tensors.size() < mpi_ctx_.neighbor_in_ranks_.size()) {
    BFLOG(ERROR, mpi_ctx_.rank_)
        << "WinCreate (NCCL) insufficient neighbor tensors for " << name
        << " expected=" << mpi_ctx_.neighbor_in_ranks_.size()
        << " got=" << entry.neighbor_tensors.size();
    entry.callback(Status::InvalidArgument(
        "Win_create: insufficient neighbor tensors for " + name +
        ", expected " + std::to_string(mpi_ctx_.neighbor_in_ranks_.size()) +
        ", got " + std::to_string(entry.neighbor_tensors.size())));
    return;
  }

  // NEW: 进一步打印每个 in-neighbor 的 rank，帮助对比
  std::ostringstream oss;
  oss << "[";
  for (size_t i = 0; i < mpi_ctx_.neighbor_in_ranks_.size(); ++i) {
    if (i > 0) oss << ",";
    oss << mpi_ctx_.neighbor_in_ranks_[i];
  }
  oss << "]";
  BFLOG(DEBUG, mpi_ctx_.rank_)
      << "WinCreate (NCCL) in_neighbor_ranks=" << oss.str();
  ...
  if (it != nccl_ctx_.named_win_map.end()) {
    BFLOG(ERROR, mpi_ctx_.rank_)
        << "WinCreate (NCCL) duplicate window name '" << name << "'";
    entry.callback(
  ...
  BFLOG(DEBUG, mpi_ctx_.rank_)
      << "WinCreate (NCCL) calling InitializeWinMemory for '" << name << "'";
...
Status NCCLController::WinFreeReturnStatus(TensorTableEntry& entry) {
  ...
  BFLOG(DEBUG, mpi_ctx_.rank_)<< "进入nccl执行win_free"<< " window_id=" << window_id;
  it->second->DestroyMutexWin();
  BFLOG(DEBUG, mpi_ctx_.rank_)<< "destory mpi_win";
  it->second->FreeWindow();
  BFLOG(DEBUG, mpi_ctx_.rank_)<< "memory free";
  nccl_ctx_.named_win_map.erase(it);
  BFLOG(DEBUG, mpi_ctx_.rank_)<< "进入nccl执行win_free成功";
...
// void NCCLController::WinPut1(TensorTableEntry& entry) {
  // We need to explicitly set the device here.
//   with_device device_guard(entry.device);
...
//   if (it == nccl_ctx_.named_win_map.end()) {
//      BFLOG(ERROR, mpi_ctx_.rank_)
//         << "WinPut (NCCL) cannot find window '" << name
//         << "' in named_win_map";
...
//   for (auto kv : entry.dst_weights) { //这个是传进来的out_neighbor，会有检查，不会有自己在
//       BFLOG(DEBUG, mpi_ctx_.rank_)
//           << "尝试发送mpi消息" <<mpi_ctx_.rank_;
...
//                 (rand() + (mpi_ctx_.rank_ * 123)) % 10;
//             std::this_thread::sleep_for(std::chrono::microseconds(random_usec));
            
//           }
//         } while (res_buf[0] != BFWinPassiveSuccess); //接收消息，表明进入其他passive消息队列
//         //两个检查
...
//     //       }
//     //      {
//     //   auto target_tensor = nccl_win_manager->GetAssociateTensorByRank(target_rank);
//     //   int target_elems = target_tensor->shape().num_elements();
//     //   if (target_elems != num_elements || target_tensor->dtype() != entry.tensor->dtype()) {
//     //     throw std::runtime_error(
//     //         std::string("WinPut (NCCL) size/dtype mismatch for ") + entry.tensor_name +
//     //         ": send numel=" + std::to_string(num_elements) +
//     //         ", target numel=" + std::to_string(target_elems));
//     //   }
//     // }
  
//   }

//   BFLOG(DEBUG, mpi_ctx_.rank_) << "all neighbor have get message ,the put rank is"<< mpi_ctx_.rank_;
//   for (auto kv : entry.dst_weights) { //接收邻居节点的拥锁消息
//     int target_rank = kv.first;

//     std::vector<int> ready_buf(1, -1);
//     MPICHECK(MPI_Recv(ready_buf.data(), 1, MPI_INT, target_rank,
//                             kWinPassiveRecvAckTag, MPI_COMM_WORLD,
//                             MPI_STATUS_IGNORE));
//     if (ready_buf[0]==BFWinPassiveSuccess){
//       //入队
//       continue;
//     } 
//   }
//     BFLOG(DEBUG, mpi_ctx_.rank_) << "all neighbor have get lock ,the put rank is"<< mpi_ctx_.rank_;
...
//     //尝试上锁
//     std::unique_lock<std::mutex> lock_win_passive(nccl_ctx_.nccl_win_mutex,std::defer_lock);
//     // const int timeout = 100;
//     while(true){
//       if (lock_win_passive.try_lock()) {
//         BFLOG(DEBUG, mpi_ctx_.rank_)
//         << "WinPut has locked" <<mpi_ctx_.rank_;
//         break;
//       }else{
//         BFLOG(DEBUG, mpi_ctx_.rank_)
//         << "due locked, waiting" <<mpi_ctx_.rank_;
//       unsigned int random_usec_key = 5;        
//       std::this_thread::sleep_for(std::chrono::microseconds(random_usec_key));
//       }
//     }
    
//     // 此次还有本进程的内存管理的锁需要考虑
//     while(win_lock.use==1){//表示有人在self锁中写入
//        std::this_thread::sleep_for(std::chrono::microseconds(2));
//     }
//      win_lock.use=1;
//     ncclGroupStart();
...
//         #endif
//          BFLOG(DEBUG, mpi_ctx_.rank_)
//       << "WinPut (NCCL) ncclSend finished to target_rank=" << target_rank
//       << " for window '" << name << "'";
//     }
//     // BFLOG(DEBUG, mpi_ctx_.rank_)
//     //   << "WinPut (NCCL) ncclSend finished to target_rank=" << target_rank
...
//    // NEW: 收到所有 done 的日志
//   BFLOG(DEBUG, mpi_ctx_.rank_)
//     << "WinPut (NCCL) received all done signals for window '" << name
//     << "' from dst_ranks=" << entry.dst_weights.size();
...
//   CUDACHECK(this->nccl_ctx_.ReleaseCudaEvent(event));
//   this->timeline_ptr_->ActivityStart(entry.tensor_name, "CALLBACK", &tid);
//   BFLOG(DEBUG, this->mpi_ctx_.rank_)
//       << "Win_Put(NCCL) for " << entry.tensor_name << " is done.";
//   entry.callback(Status::OK());
//   BFLOG(DEBUG, mpi_ctx_.rank_)
//       << "entry callback " << entry.tensor_name << " is done.";
//   this->timeline_ptr_->ActivityEnd(entry.tensor_name, &tid);
// }
...
void NCCLController::WinPut(TensorTableEntry& entry){
  ...
  for (auto kv : entry.dst_weights) {
    ...
    // while (!nccl_ctx_.nccl_win_mutex.try_lock()) {
    // // 成功获取锁，使用 std::unique_lock 确保 RAII
    // BFLOG(DEBUG, mpi_ctx_.rank_) << "unlock put" << mpi_ctx_.rank_;
    ...
    auto& win_stream = nccl_ctx_.nccl_win_active_streams[mpi_ctx_.rank_]; 
            ...
            NCCLCHECK(ncclBroadcast(sendbuf, /*recvbuf=*/nullptr, num_elements,
                                    ...
                                    /*root=*/0, win_comm, win_stream));
        #endif
    BFLOG(DEBUG, mpi_ctx_.rank_)<< "WinPut  ncclSend finished to target_rank=" << target_rank ;
    // ncclGroupEnd();
    // nccl_ctx_.nccl_win_mutex.unlock();//释放锁
  };
  
  //第三个部分，确认所有的消息都发送完毕，然后确认结束代码
    std::vector<MPI_Request> requests(entry.dst_weights.size());
    std::vector<MPI_Status> statuses(entry.dst_weights.size());
    // done 数组用于接收目标 Rank 发来的完成信号
    std::vector<int> done(entry.dst_weights.size(), -1); 
    int count = 0;

    // 1. 启动所有非阻塞接收 (MPI_Irecv)
    for (auto kv : entry.dst_weights) {
      int target_rank = kv.first;
      MPICHECK(MPI_Irecv(done.data() + count, 1, MPI_INT, target_rank,
                        kWinPassiveDoneTag, MPI_COMM_WORLD, &requests[count]));
...
void NCCLController::WinAccumulate(TensorTableEntry& entry) {
  ...
  for (auto kv : entry.dst_weights) {
    ...
    //   timeline_ptr->ActivityEnd(entry.tensor_name);
    // }
     // NEW: Validate target window tensor compatibility to avoid memory corruption.
    {
      auto target_tensor = nccl_win_manager->GetAssociateTensorByRank(target_rank);
      int target_elems = target_tensor->shape().num_elements();
      if (target_elems != num_elements ||
          target_tensor->dtype() != entry.tensor->dtype()) {
        throw std::runtime_error(
            std::string("WinAccumulate (NCCL) size/dtype mismatch for ") + entry.tensor_name +
            ": send numel=" + std::to_string(num_elements) +
            ", target numel=" + std::to_string(target_elems));
      }
    }
