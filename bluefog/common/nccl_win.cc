...
bool NCCLWindowManager::InitializeWinMemory(
    ...
    const MPIContext& mpi_ctx) {
       ...
       // 添加安全检查
    if (!tensor) {
        BFLOG(ERROR) << "InitializeWinMemory: tensor is null";
        return false;
    }
    
    if (!tensor->data()) {
        BFLOG(ERROR) << "InitializeWinMemory: tensor data is null";
        return false;
    }
 ...
 // 调试：打印基础信息
  BFLOG(DEBUG) << "InitializeWinMemory: device=" << device
               << " mpi_rank=" << mpi_ctx.rank_
               << " mpi_size=" << mpi_ctx.size_
               << " neighbor_in_ranks_.size=" << mpi_ctx.neighbor_in_ranks_.size()
               << " neighbor_tensors.size=" << neighbor_tensors.size();

  // 防御性检查：neighbor_tensors 数量是否和 in-neighbors 一致
  auto expected_neighbors = mpi_ctx.neighbor_in_ranks_.size();
  if (neighbor_tensors.size() != expected_neighbors) {
    BFLOG(ERROR) << "InitializeWinMemory: neighbor_tensors size mismatch for rank "
                 << mpi_ctx.rank_
                 << " (expected=" << expected_neighbors
                 << ", actual=" << neighbor_tensors.size() << ")";
    throw std::runtime_error(
        "NCCLWindowManager::InitializeWinMemory neighbor_tensors size mismatch");
  }
...
//新加入的
  ...
  wins_tensor_vec_.reserve(mpi_ctx.size_);
...
// 检查为什么内存报错
    bool is_self = (rank == mpi_ctx.rank_);
    bool is_in_neighbor = (std::find(mpi_ctx.neighbor_in_ranks_.begin(),
                                     mpi_ctx.neighbor_in_ranks_.end(),
                                     rank) != mpi_ctx.neighbor_in_ranks_.end());

    if (is_self) {
      BFLOG(DEBUG) << "InitializeWinMemory: rank=" << mpi_ctx.rank_
                   << " add self tensor (global_rank=" << rank << ")";
      wins_tensor_vec_.push_back(tensor);
    } else if (is_in_neighbor) {
      if (neighbor_tensor_index >= static_cast<int>(neighbor_tensors.size())) {
        BFLOG(ERROR) << "InitializeWinMemory: neighbor_tensor_index out of range at rank "
                     << mpi_ctx.rank_ << " for neighbor global_rank=" << rank
                     << " (idx=" << neighbor_tensor_index
                     << ", size=" << neighbor_tensors.size() << ")";
        throw std::runtime_error(
            "NCCLWindowManager::InitializeWinMemory neighbor index overflow");
      }
      BFLOG(DEBUG) << "InitializeWinMemory: rank=" << mpi_ctx.rank_
                   << " add neighbor tensor for global_rank=" << rank
                   << " (idx=" << neighbor_tensor_index << ")";
      wins_tensor_vec_.push_back(neighbor_tensors[neighbor_tensor_index++]);
    } else {
      BFLOG(DEBUG) << "InitializeWinMemory: rank=" << mpi_ctx.rank_
                   << " add placeholder for non-neighbor global_rank=" << rank;
