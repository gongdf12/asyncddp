...
namespace bluefog {
namespace common {
comm_thread comm_thread_status={0,0};
Mutex win_mutex={-1,0,0};
Write_read_lock win_lock={0};
Status MutexRelease() {
...
// 进程内部读写互斥量
// struct Mutex{
//   int target_rank; // 用于检查目标rank的内存区是否在修改
//   int mutex;       // 表明现在是否有修改动作
// }
// Mutex win_mutex={-1,0};
...
bool RunLoopOnce(BluefogGlobalState& state);//这个只是定义，实现在
...
void BackgroundThreadLoop(BluefogGlobalState& state) {
  ...
  state.controller->Initialize();  //初始化mpi控制器和上下文
  ...
  if (bluefog_fusion_threshold != nullptr) {
    ...
    state.tensor_fusion_threshold_for_dst_weight =
        state.tensor_fusion_threshold;
  }//读取 BLUEFOG_CYCLE_TIME / BLUEFOG_FUSION_THRESHOLD 等环境变量，覆盖默认的循环周期和 tensor fusion 阈值
...
Vendor DetermineController(const MPIOpsType& op_type, int device) {
  if (device == CPU_DEVICE_ID) {
    BFLOG(DEBUG) << "DetermineController: op_type=" << static_cast<int>(op_type)
                 << " device=" << device
                 << " -> vendor=" << Vendor_Name(Vendor::MPI)
                 << " (CPU_DEVICE_ID)";
    return Vendor::MPI;
  }
  ...
  bool built_with_nccl = false;
      ...
      switch (op_type) {
        ...
        case MPIOpsType::WIN_READ_NEIGHBOR:
      ...
      Vendor vendor;
      if (!built_with_nccl || !nccl_impl_available || force_mpi) {
        vendor = Vendor::MPI;
      } else {
        vendor = Vendor::NCCL;
      }
      ...
      BFLOG(DEBUG) << "DetermineController: op_type=" << static_cast<int>(op_type)
                  << " device=" << device
                  << " built_with_nccl=" << built_with_nccl
                  << " nccl_available=" << nccl_impl_available
                  << " force_mpi=" << force_mpi
...
void PerformOperation(std::vector<TensorTableEntry>& entries) {
  ...
  for (auto& entry : entries) {
    ...
    BFLOG(DEBUG)<<"do performoperation label is"<< index;
    index++;
    switch (entry.mpi_ops_type) {
              ...
              case MPIOpsType::WIN_READ_NEIGHBOR:
                  BFLOG(TRACE, bluefog_global.controller->GetRank())
                      << "Processing WIN_READ_NEIGHBOR on " << entry.tensor_name
                      << " with " << Vendor_Name(controller_vendor);
                  timeline.ActivityStart(entry.tensor_name, "WIN_READ_NEIGHBOR");
                  if (controller_vendor == Vendor::MPI) {
                    bluefog_global.controller->WinReadNeighbor(entry);
                  }
                  #if HAVE_NCCL
                          if (controller_vendor == Vendor::NCCL) {
                            entry.callback(Status::PreconditionError(
                                "WIN_READ_NEIGHBOR is not supported with NCCL."));
                          }
                  #endif
                          timeline.ActivityEnd(entry.tensor_name);
                          break;
...
bool RunLoopOnce(BluefogGlobalState& state) {
  ...
  BFLOG(DEBUG) << "开始取消息";
  std::deque<Request> message_queue_buffer;
  state.tensor_queue.PopMessagesFromQueue(message_queue_buffer);//从全局消息队列中取出消息到本地buff
  BFLOG(DEBUG) << "已经取出消息从全局队列  " << message_queue_buffer.size();
  ...
  };//定义一个 lambda，用于判断“这个 request 是否可以直接转成 TensorTableEntry 执行，不需要 rank 间协商”：
  ...
  if (global_skip_negotiate_stage) {
    ...
    BFLOG(DEBUG) << "NegotiationOfRequest begin, buffer size=" << message_queue_buffer.size();
    NegotiationOfRequest(state, message_queue_buffer, should_change_topo,
                         should_shut_down);
    BFLOG(DEBUG) << "NegotiationOfRequest end";                      
  }
...
Status EnqueueTensorWindowCreate(
  ...
  if (bluefog_global.shut_down) {
    BFLOG(ERROR) << "EnqueueTensorWindowCreate failed for window '" << name
                 << "': bluefog_global.shut_down is true";
    return SHUT_DOWN_ERROR;
  }
  if (global_background_thread_suspend) {
    BFLOG(ERROR) << "EnqueueTensorWindowCreate failed for window '" << name
                 << "': background thread is suspended";
    return SUSPEND_ERROR;
  }
  Status status = bluefog_global.tensor_queue.AddToTensorQueue(e, message);
  
  if (!status.ok()) {
    BFLOG(ERROR, bluefog_global.controller->GetRank())
        << "EnqueueTensorWindowCreate AddToTensorQueue failed for window '"
        << name << "' on device " << device
        << " : " << status.reason();
  } else {
    BFLOG(DEBUG, bluefog_global.controller->GetRank())
        << "EnqueueTensorWindowCreate succeeded for window '"
        << name << "' on device " << device;
  }
...
Status EnqueueTensorWindowPut(std::shared_ptr<Tensor> tensor,
  ...
  BFLOG(DEBUG)<<"EnqueueTensorWindowPut";
  Status status = bluefog_global.tensor_queue.AddToTensorQueue(e, message);//入队
  {
    std::unique_lock<std::mutex> lk(bluefog_global.loop_mutex);
    bluefog_global.loop_cv.notify_one();//通知background有新的任务
  }
  BFLOG(DEBUG)<<"入队完成";
  return status;
}
...
// Status EnqueueTensorWindowReadNeighbor(
//     const std::string& name,
//     int neighbor_rank,
//     std::shared_ptr<Tensor> output,
//     const int device,
...
//   Request message;
//   message.set_request_rank(bluefog_global.controller->GetRank());
//   // 每个 (name, neighbor_rank) 一个独立 message 名称，避免 DUPLICATE_NAME_ERROR
//   message.set_tensor_name("win_read_neighbor." + name + "." +
//                           std::to_string(neighbor_rank));
//   // 用 WIN_GET 这种非协调型 request_type，使其直接走 tensor_queue
//   message.set_request_type(Request::WIN_GET);

//   TensorTableEntry e;
//   e.tensor_name = name;                      // 纯窗口名
//   e.output = output;
//   e.callback = callback;
//   e.device = device;
//   e.mpi_ops_type = MPIOpsType::WIN_READ_NEIGHBOR;
//   e.require_mutex = require_mutex;
//   e.src_weights.clear();
//   e.src_weights.emplace(neighbor_rank, 1.0);  // 用 key 存邻居 rank
