...
// Status EnqueueTensorWindowReadNeighbor(const std::string& name,
//                                         int neighbor_rank,
//                                         std::shared_ptr<Tensor> output,
//                                         const int device,const bool require_mutex,
//                                         StatusCallback callback);
// 暴露进程内部读写锁的接口
struct Mutex {
  int target_rank; //表明现在某个rank的窗口内存在被修改
  int use;   //表明是否有修改操作
  int global;
}; //这个用于对付本地进程中存在的被动写入-读出冲突，保证内存只有一个线程访问，update时没有被动线程写入
struct comm_thread{
  int ready_passive;
  int ready_win;
}; //这个结构体用来处理死锁之后的死锁的释放
extern comm_thread comm_thread_status;
extern Mutex win_mutex;
Status MutexRelease(); //关于上锁的方式每个线程要求不一样，但是锁的释放每个线程是一样的
struct Write_read_lock{
    int use; //表示self 的内存数据是否被使用
};//用于处理winput与update冲突，避免在winput时内存被其他线程修改。
extern Write_read_lock win_lock;
