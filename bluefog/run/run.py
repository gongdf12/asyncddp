...
def parse_args():
    ...
    parser.add_argument('--use-ucx', action='store_true', dest='use_ucx',
                        help='Use UCX (pml ucx) instead of OB1. Can also set BLUEFOG_USE_UCX=1')
...
def main():
    ...
    # )
     # UCX switch via flag or env var
    use_ucx = bool(args.use_ucx) or (os.environ.get('BLUEFOG_USE_UCX', '0') == '1')
    if use_ucx:
        # Set sensible UCX defaults if not provided by user
        env.setdefault('UCX_TLS', 'cuda_copy,sm,self,tcp')
        env.setdefault('UCX_MEMTYPE_CACHE', 'n')
        # Prefer user-provided NIC, else common interfaces derived above
        if args.nic:
            env.setdefault('UCX_NET_DEVICES', args.nic)
        elif common_intfs:
            env.setdefault('UCX_NET_DEVICES', ','.join(common_intfs))
        mpirun_command = (
            '{prefix}mpirun --allow-run-as-root '
            '-np {num_proc} {hosts_arg} '
            '-bind-to none -map-by slot '
