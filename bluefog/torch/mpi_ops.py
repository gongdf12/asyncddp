...
from bluefog.common import util
...
is_available = _basics.is_available
...
# -----------------------------------------------------------------------------
# Object broadcast helpers (pickle-based)
# -----------------------------------------------------------------------------
def _object_to_tensor(obj: Any, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
    """Serialize a Python object to a uint8 tensor and a size tensor on `device`."""
    payload = pickle.dumps(obj, protocol=pickle.HIGHEST_PROTOCOL)
    arr = np.frombuffer(payload, dtype=np.uint8)
    t = torch.from_numpy(arr.copy())  # ensure writeable & contiguous
    if device.type != 'cpu':
        t = t.to(device, non_blocking=False)
    size_t = torch.tensor([t.numel()], dtype=torch.long, device=device)
    return t, size_t
def _tensor_to_object(t: torch.Tensor) -> Any:
    """Deserialize a Python object from a uint8 tensor located on any device."""
    buf = t.detach().to('cpu', non_blocking=False).numpy().tobytes()
...
def broadcast_object_list(
    ...
    """
    Broadcast a list of picklable Python objects from `src` rank to all ranks.
    - All ranks must provide `object_list` with identical length.
    - Uses CPU by default for efficiency with byte tensors.
    - This call is blocking and updates `object_list` in place on non-src ranks.
    """
    if src is None:
        src = 0
    current_device = device or torch.device('cpu')
    ...
    if rank() == src:
        ...
        sizes = torch.empty(len(object_list), dtype=torch.long, device=current_device)
    # Broadcast sizes first
    broadcast_(sizes, root_rank=src, name=(f"{name}.sizes" if name is not None else None))
    # Prepare concatenated payload
    if rank() == src:
        if len(tensor_list) == 1:  # type: ignore[name-defined]
            data = tensor_list[0]  # type: ignore[name-defined]
        else:
            data = torch.cat(tensor_list)  # type: ignore[name-defined]
    else:
        total_n = int(sizes.sum().item()) if sizes.numel() > 0 else 0
        data = torch.empty(total_n, dtype=torch.uint8, device=current_device)
    # Broadcast data buffer
    broadcast_(data, root_rank=src, name=(f"{name}.data" if name is not None else None))
    # Deserialize on non-src ranks and write back in place
    if rank() != src:
        offset = 0
        for i in range(len(object_list)):
            sz = int(sizes[i].item())
            if sz == 0:
                # Zero-length payload is unlikely with pickle but guard anyway
                object_list[i] = None
                continue
            obj_view = data[offset : offset + sz]
            offset += sz
            object_list[i] = _tensor_to_object(obj_view)
def broadcast_object(
    ...
    """Broadcast a single picklable object. Returns the object on all ranks."""
    lst = [obj] if rank() == src else [None]
    broadcast_object_list(lst, src=src, device=device, name=name)
    return lst[0]
...
def win_update(name: str,
    ...
    timeout_s = util.win_timeout_seconds()
    warn_after_s = util.win_warn_after_seconds()
    # Guard the potentially blocking C-extension with a watchdog that restarts
    # the process on timeout to avoid silent deadlocks.
    with util.watchdog_then_restart(timeout_s, warn_after_s, label=f"win_update({name})"):
        ok = getattr(mpi_lib, function)(
            tensor,
            name,
            self_weight,
            neighbor_weights,
...
def win_read_neighbor(name: str, rank_: int, require_mutex: bool = True) -> torch.Tensor:
    """Read the window buffer contributed by a single neighbor rank.

    This returns a cloned tensor snapshot of the neighbor slot corresponding
    to `rank_` for the given window `name`. It is intended for advanced
    algorithms (e.g., version-aware per-neighbor aggregation) that need
    fine-grained control over neighbor contributions.
    """
    return mpi_lib.bluefog_torch_win_read_neighbor(name, rank_, require_mutex)
...
def win_cancel(handle: int) -> None:
    """逻辑取消一个 window handle，不再等待它完成。"""
    if handle not in _win_handle_map:
        return
    mpi_lib.bluefog_torch_win_cancel(handle)
    _win_handle_map.pop(handle, None)
def win_wait(handle: int) -> bool:
    ...
    if handle not in _win_handle_map:
        ...
        logger.warning(
            "Win wait is called but the handle is not found in the _win_handle_map."
        )
        return False
        # return False
    ...
    while True:
        if win_poll(handle):
            # Finalize handle via C call which should return promptly now.
            mpi_lib.bluefog_torch_win_wait(handle)
            _ = _win_handle_map.pop(handle)
            return True
        ...
        if not warned and elapsed >= warn_after_s:
            logger.warning(
                f"[rank {rank()}] win_wait appears stuck for {int(elapsed)}s; monitoring…"
            )
            warned = True
        if elapsed >= timeout_s:
            logger.error(
                f"[rank {rank()}] win_wait timed out after {int(elapsed)}s; restarting…"
            )
            util.restart_self_or_exit()
            # restart_self_or_exit() does not return; this is for type completeness.
            return False
        time.sleep(0.1)
...
def get_current_created_window_names() -> List[str]:
    ...
    # with util.watchdog_then_restart(util.win_timeout_seconds(), util.win_warn_after_seconds(),
    #                                 label=f"win_lock({name})"):
    #     mpi_lib.bluefog_torch_win_lock(name)
...
def _win_mutex_acquire(name, ranks, for_self):
    with util.watchdog_then_restart(util.win_timeout_seconds(), util.win_warn_after_seconds(),
                                    label=f"win_mutex_acquire({name})"):
...
def _win_mutex_release(name, ranks, for_self):
    with util.watchdog_then_restart(util.win_timeout_seconds(), util.win_warn_after_seconds(),
                                    label=f"win_mutex_release({name})"):
