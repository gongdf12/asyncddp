...
// Detect modern PyTorch (c10/cuda present) and gate legacy TH API
#if defined(__has_include)
#  if __has_include(<torch/version.h>)
#    include <torch/version.h>
#    if defined(TORCH_VERSION_MAJOR)
#      define BF_HAVE_C10_CUDA 1
#    else
#      define BF_HAVE_C10_CUDA 0
#    endif
...
// using ::bluefog::common::EnqueueTensorWindowReadNeighbor;
...
bool WinTorchStorageManager::AvgWithNeighbor(
  ...
  local_tensor.mul_(self_weight);  //原地操作张量
...
int DoWinCreate(::torch::Tensor tensor, const std::string& name,
  ...
  if (!enqueue_result.ok()) {
    BFLOG(ERROR) << "WIN_CREATE enqueue failed for window '" << name
                 << "' on device " << device
                 << " : " << enqueue_result.reason();
    // 保持原有行为：抛出异常
    ThrowIfError(enqueue_result);
  } else {
    BFLOG(DEBUG) << "WIN_CREATE enqueued for window '" << name
                 << "' on device " << device
                 << " (handle=" << handle << ")";
  }
...
// Expose a helper to read one neighbor's window buffer as a Torch tensor.
// This is intended for higher-level algorithms that want finer control over
// per-neighbor contributions (e.g., version-aware aggregation) without
// reimplementing window storage logic in Python.
// reimplementing window storage logic in Python.
// ::torch::Tensor DoWinReadNeighbor(const std::string& name, int rank) {
::torch::Tensor DoWinReadNeighbor(const std::string& name, int rank,
                                  bool require_mutex) {
  ThrowIfError(common::CheckInitialized());
  // std::lock_guard<std::mutex> guard(common::w_r_mutex);
  int device = CPU_DEVICE_ID;
  if (!win_storage_manager.GetDeviceByName(name, &device)) {
    throw std::runtime_error("Cannot get device of win " + name);
    
  }

  // 先查一次邻居槽，拿到 shape / dtype / device，用它来分配输出 Tensor
  std::shared_ptr<TorchTensor> neighbor_storage;//用来获取张量类型的指针
  if (!win_storage_manager.GetStorageByNameRank(name, rank, neighbor_storage)) {
    throw std::runtime_error(
        std::string("Cannot get neighbor tensor for window ") + name +
        " at rank " + std::to_string(rank));
  }//利用之前定义的函数获取了win_name下的rank张量指针

  ::torch::Tensor template_tensor = neighbor_storage->GetUnderlyingTensor().clone();
  // ::torch::Tensor out = template_tensor.clone();
  //取消线程入队操作，这个地方只需要输出name窗口下对应的rank就行，参考dowinsync
  return template_tensor; //直接返回邻居张量
}
...
int DoWinSync(::torch::Tensor tensor, const std::string& name,
  ...
  if (require_mutex){
    ...
    //检查读写互斥量，在update操作时，需要所有的rank不写入
    while (common::win_mutex.global == 1){ //有全局锁
      std::this_thread::sleep_for(std::chrono::microseconds(2)); //等待2微秒
     
    }

    common::win_mutex.global = 1; //进行全局上锁操作
    //检查是否有正在写的
    while(true){
      if (common::win_mutex.target_rank == -1){ //没有正在写的
        break;
      }else{ //正在写的rank
        std::this_thread::sleep_for(std::chrono::microseconds(2)); //等待2微秒
      }
    }//开始读
     while(common::win_lock.use == 1){
       std::this_thread::sleep_for(std::chrono::microseconds(2));
     }
     common::win_lock.use = 1;
...
static std::unordered_map<std::string, std::vector<int> > last_put_handle;//用于存储上一次没有处理完的handle
int DoWinPut(::torch::Tensor tensor, const std::string& name,
  ...
  ThrowIfError(common::CheckInitialized());
  //处理上一次的handle
  auto & list_handle = last_put_handle[name];
  // BFLOG(DEBUG) << "是否还有之前的put没有处理"<<list_handle.empty() ;
  if(!list_handle.empty()){
    int last = list_handle.back();
  if (!win_handle_manager.PollHandle(last)) {
    DoWinWait(last);           // 阻塞直到上一次 put 完成
    // BFLOG(DEBUG) << " previous put finish,第一个元素";
    list_handle.pop_back();  
  }
 } 
  Timeline* timeline_ptr;
  Status timeline_status = GetBluefogTimeline(timeline_ptr);
  ...
  // BFLOG(DEBUG) << "开始enqueue"<< ::bluefog::common::bluefog_rank();
  auto enqueue_result = EnqueueTensorWindowPut(
      ...
      [handle, name, timeline_ptr, tid, tensor, self_weight,
       associated_with_p](const Status& status) mutable {
        ...
        // BFLOG(INFO)<<"进行callback";
        win_handle_manager.MarkDone(handle, status);
        // BFLOG(INFO)<<"callback结束";
...
static std::unordered_map<std::string, std::vector<int> > last_Accumulate_handle;//用于存储上一次没有处理完的handle
int DoWinAccumulate(::torch::Tensor tensor, const std::string& name,
  ...
  auto & list_handle = last_Accumulate_handle[name];
  // BFLOG(DEBUG) << "是否还有之前的put没有处理"<<list_handle.empty() ;
  if(!list_handle.empty()){
    int last = list_handle.back();
  if (!win_handle_manager.PollHandle(last)) {
...
static std::unordered_map<std::string, std::vector<int> > last_get_handle;//用于存储上一次没有处理完的handle
int DoWinGet(const std::string& name,
  ...
  auto & list_handle = last_get_handle[name];
  // BFLOG(DEBUG) << "是否还有之前的put没有处理"<<list_handle.empty() ;
  if(!list_handle.empty()){
    int last = list_handle.back();
  if (!win_handle_manager.PollHandle(last)) {
...
void AddWinOpsIntoPybind(py::module& m) {
  ...
  // m.def("bluefog_torch_win_cancel", &DoWinCancel);  // 新增
  ...
  // Per-neighbor window buffer reader for fine-grained algorithms.
  m.def("bluefog_torch_win_read_neighbor", &DoWinReadNeighbor);
}
