...
class _DistributedOptimizer(torch.optim.Optimizer): # 这个类用于更新通信梯度
    ...
    def _make_hook(self, p): # 递减倒计时到0调用_allreduce_grad_async
        def hook(*ignore):
...
class _DistributedReduceOptimizer(torch.optim.Optimizer):  # 这个用于通信模型的参数
