...
def get_cpp_flags(build_ext):
    last_err = None
    # Try C++17 first for newer PyTorch headers; fall back to C++14.
    base_flags17 = ['-std=c++17', '-fPIC', '-O2', '-Wall']
    base_flags14 = ['-std=c++14', '-fPIC', '-O2', '-Wall']
    if sys.platform == 'darwin':
        # Darwin most likely will have Clang, which has libc++.
        flags_to_try = [base_flags17 + ['-stdlib=libc++'], base_flags17,
                        base_flags14 + ['-stdlib=libc++'], base_flags14]
    else:
        flags_to_try = [base_flags17, base_flags14]
...
def get_link_flags(build_ext):
    # Restrict special link flags to macOS only; on Linux/Unix, none needed.
    if sys.platform == 'darwin':
        last_err = None
        flags_to_try = [['-Wl,-exported_symbols_list'], []]
...
def is_torch_cuda(build_ext, include_dirs, extra_compile_args):
    # Check PyTorch headers are usable. CUDA availability is checked via torch.cuda.
    try:
        from torch.utils.cpp_extension import include_paths
        torch_includes = include_paths()
        test_compile(build_ext, 'test_torch_headers',
                     include_dirs=include_dirs + torch_includes,
        ...
        print('INFO: Above error indicates that PyTorch C++ headers are not available or incompatible.')
        return False
...
setup(
    ...
    setup_requires=['numpy>=1.17', 'cffi>=1.4.0', 'packaging>=20.0'],
